...
pub struct SimulationDiagnostics {
    ...
    // Consolidated trackers (replacing separate tracker resources)
    /// Expected mass delta from sources/sinks since last check [kg]
    pub expected_mass_delta: f32,
    /// Last total mass sampled for conservation monitoring [kg]
    pub last_total_mass: f32,
    /// Last momentum components sampled for conservation monitoring [kg·m/s]
...
impl SimulationDiagnostics {
    ...
    pub fn update(
        ...
        self.compression_heating_rate =
            -(constants::GAMMA - 1.0) * self.avg_temperature * self.avg_div_u;
